    ...
    case DECORPERM_PASS_FLOOR:
        for (i = 0; i < mapY; i++)
        {
            ...
            for (j = 0; j < mapX; j++)
            {
                ...
                behaviorAt = GetObjectEventIdByPosition(curX, curY, ELEVATION_MATCH_ANY);
    ...
    case DECORPERM_BEHIND_FLOOR:
        for (i = 0; i < mapY - 1; i++)
        {
            ...
            for (j = 0; j < mapX; j++)
            {
                ...
                if (GetObjectEventIdByPosition(curX, curY, ELEVATION_MATCH_ANY) != OBJECT_EVENTS_COUNT)
                    return FALSE;
            }
        }
...
static void PlaceDecoration(u8 taskId)
{
    ...
    if (GetActiveLocationData()->regionMapSectionId == MAPSEC_SECRET_BASE)
