    ...
    for (j = 0; j < 10; j++)
    {
        for (i = 0; i < 15; i++)
        {
            ...
            if (metatile < NUM_METATILES_IN_PRIMARY)
                ...
                BuyMenuDrawMapMetatile(i, j, GetActiveLocationData()->secondaryTileset->metatiles + ((metatile - NUM_METATILES_IN_PRIMARY) * NUM_TILES_PER_METATILE), metatileLayerType);
        }
    }
}
