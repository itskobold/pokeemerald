...
void InitBattleControllers(void)
{
    ...
    // Seed the shared battle RNG (exchanged between link players) and identify the local player
    // in link battles. Formerly done by RecordedBattle_SetTrainerInfo in recording mode.
...
void BtlController_EmitLinkStandbyMsg(u8 bufferId, u8 mode, bool32 record)
{
    ...
    // Recorded battles removed; no recorded battler data is appended (both link players agree on 0).
...
void BtlController_EmitEndLinkBattle(u8 bufferId, u8 battleOutcome)
{
    ...
    // Recorded battles removed; no recorded battler data is appended.
    sBattleBuffersTransferData[5] = sBattleBuffersTransferData[4] = 0;
